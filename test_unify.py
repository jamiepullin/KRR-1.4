...
def run_tests():
    ...
    print("\n---Occur-check Test---")
    Y_var2 = V("Y")
    circular_term = C("f", [C("g", [Y_var2])])  # f(g(Y))
    theta_occur = unify(Y_var2, circular_term, {})
    if theta_occur is None:
        print("Occur-check test: PASS (unification correctly failed)")
    else:
        print("Occur-check test: FAIL (unification should not succeed)")
        print(f"Resulting θ: {print_theta(theta_occur)}")
